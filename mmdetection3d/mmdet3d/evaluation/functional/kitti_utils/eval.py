...
# Calculate F1-score
    result += '\n----------- F1-score Results ------------\n\n'
    for j, curcls in enumerate(current_classes):
        # F1-score result: [num_class]
        curcls_name = class_to_name[curcls]
        f1_scores = compute_f1_score(preds, gts, ignores=[])  # Modify the ignores list if needed

        # Log F1-scores for each class
        result += '{} F1-score: {:.4f}\n'.format(curcls_name, f1_scores[j])

        # prepare results for logger
        prefix = f'KITTI/{curcls_name}'
        ret_dict[f'{prefix}_F1_score'] = f1_scores[j]
    # Calculate F1 score
    # Assuming you have true_labels and pred_labels for each class
    true_labels = [1, 1, 0, 1, 0, 0, 1, 0, 1]  # Replace with your actual ground truth labels
    pred_labels = [1, 0, 1, 1, 0, 1, 0, 0, 1]  # Replace with your actual predicted labels

    f1 = f1_score(true_labels, pred_labels, average='weighted')

    # Log the results
    log_str = f"Overall mAP: {overall_mAP}, F1 Score: {f1}"
    print(log_str)
