...
class DataBaseSampler(object):
    ...
    def sample_class_v2(self, name: str, num: int,
        ...
        num_sampled = len(sampled)
        if not sampled:
            print("Warning: 'sampled' is empty. Handle this case appropriately.")
            return []

        gt_bboxes_bv = box_np_ops.center_to_corner_box2d(
            gt_bboxes[:, 0:2], gt_bboxes[:, 3:5], gt_bboxes[:, 6])
        # Before the problematic line
        # Inside the sample_class_v2 method, before the problematic line

        print("Contents of sampled:", sampled)
