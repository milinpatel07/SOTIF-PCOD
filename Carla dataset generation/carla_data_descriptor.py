...
class CarlaDataDescriptor:
    ...
    def set_3d_object_dimensions(self, bbox_extent):
        ...
        height, width, length = round(bbox_extent.z, 2), round(bbox_extent.y, 2), round(bbox_extent.x, 2)
    ...
    def set_3d_object_location(self, actor, sensor, actor_type):
        # project location from carla world coordinate to camera coordinate
        actor_transform = actor.get_transform()

        location_x = round(actor_transform.location.x - sensor.location.x, 2)
        location_y = round(actor_transform.location.y - sensor.location.y, 2)
        if actor_type == "Car":
            location_z = round(actor_transform.location.z - sensor.location.z, 2)
        else:
            location_z = round(actor.bounding_box.location.z - sensor.location.z, 2)
        # since the actor.get_location().z will reture the center point of bounding box for pedestrian but the lowest point for car
        
        # print("actor_location.z", actor_location.z)
        # print("actor.bounding_box.location.z", actor.bounding_box.location.z)
        # print("actor_bbx_extent.z", actor.bounding_box.extent.z)
        # print("sensor.location", sensor.location.z)
        # print("location in vehicle coor without rotate", location_x, location_y, location_z)
        R = rotz(-1 * (sensor.rotation.yaw * pi/ 180 ))
        # print("sensor.rotation.yaw", sensor.rotation.yaw)
        # print("actor.rotation.yaw", actor_transform.rotation.yaw)
        [location_x, location_y, location_z] = np.dot(R, np.transpose([location_x, location_y, location_z]))
        # print("location in vehicle coor with rotate", location_x, location_y, location_z)
        self.location = " ".join(map(str, [location_y, -1 * location_z, location_x]))


    def set_3d_object_location_old(self, obj_location):
        """ TODO: Change this to
    ...
    def __str__(self):
        ...
        return "{} {} {} {} {} {} {} {} {}".format(self.type, self.truncated, self.occluded, self.alpha, bbox_format, self.dimensions, self.location, self.rotation_y, self.id)
