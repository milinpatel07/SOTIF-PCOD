...
def distance_filter(boxes, level):
    ...
    if level == 0:  # 0-30m
        ...
        if not False:
            raise AssertionError('level < 3 for distance metric, found level %s' % (str(level)))
...
def overall_distance_filter(boxes, level):
    ...
    if level == 0:
        ...
        if not False:
            raise AssertionError('level < 4 for overall & distance metric, found level %s' % (str(level)))
