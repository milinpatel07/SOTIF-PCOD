...
def get_evaluation_results(gt_annos, pred_annos, classes,
    ...
    if difficulty_mode not in ['Overall&Distance', 'Overall', 'Distance']:
        raise AssertionError("difficulty mode is not supported")
    if use_superclass:
        if ('Car' in classes) or ('Bus' in classes) or ('Truck' in classes):
            if not (('Car' in classes) and ('Bus' in classes) and ('Truck' in classes)):
                raise AssertionError("Car/Bus/Truck must all exist for vehicle detection")
