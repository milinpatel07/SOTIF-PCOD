...
class CadcDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        if root_path is not None: # Image sets information
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        if 'annos' not in self.cadc_infos[0].keys():
            raise AssertionError
        ...
        for i, item in enumerate(eval_gt_annos):
            boxes3d_lidar = np.array(item['gt_boxes_lidar'])
            ...
            boxes3d_lidar[:,0] *= -1
            item['location'] = boxes3d_lidar[:,:3]
            item['dimensions'] = boxes3d_lidar[:,3:6]
            item['rotation_y'] = boxes3d_lidar[:,6]
            ...
            bbox = []
            for j in range(len(item['bbox'])):
                bbox.append(np.array([0,0,50,50]))
            item['bbox'] = bbox

        for i, item in enumerate(eval_det_annos):
            boxes3d_lidar = np.array(item['boxes_lidar'])
            boxes3d_lidar = boxes3d_lidar[:,[1,0,2,3,4,5,6]]
            boxes3d_lidar[:,0] *= -1
            item['location'] = boxes3d_lidar[:,:3]
            item['dimensions'] = boxes3d_lidar[:,3:6]
