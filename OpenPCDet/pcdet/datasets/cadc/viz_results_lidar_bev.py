...
def draw_bev(lidar, annotations, predictions, output_path, s1=50,s2=50,f1=50,f2=50):
    ...
    for i, item in enumerate(lidar_x):
        if item > fwd_range[0] and item < fwd_range[1]: #get the lidar coordinates
            if lidar_y[i] > side_range[0] and lidar_y[i] < side_range[1]:

                lidar_x_trunc.append(item)
...
def main():
    ...
    for viz_idx in viz_set:
        ...
        # filter prediction
        predictions = []
        for i, item in enumerate(result_frame['boxes_lidar']):
