...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        for db_info_path in sampler_cfg.DB_INFO_PATH:
            ...
            if not db_info_path.exists():
                if 'BACKUP_DB_INFO' in sampler_cfg:
                    print("Using BACKUP_DB_INFO paths.")
                    ...
                    # Handle missing BACKUP_DB_INFO case here
                    # For example, log an error and halt initialization or provide a default path
                    print("Error: Backup database information (BACKUP_DB_INFO) not provided in configuration.")
    ...
    def load_db_to_shared_memory(self):
        ...
        if self.sampler_cfg.DB_DATA_PATH.__len__() != 1:
            raise AssertionError('Current only support single DB_DATA')
    ...
    @staticmethod
    def copy_paste_to_image_kitti(data_dict, crop_feat, gt_number, point_idxes=None):
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None,
        ...
        if self.sampler_cfg.get('USE_ROAD_PLANE', False) and mv_height is None:
            ...
            # Handle the missing 'road_plane' case
            # For example, log a warning or use a default road plane
            print("Warning: 'road_plane' key not found in data_dict. Skipping road plane adjustment.")
            # If using a default road plane is appropriate, define it here
            # Example: default_road_plane = [0, -1, 0, 0]  # Adjust based on your application needs
            # And then use it in place of data_dict['road_plane']
            data_dict.pop('calib')
            data_dict.pop('road_plane')
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            ...
            if self.sampler_cfg.get('USE_ROAD_PLANE', False) and mv_height is not None:
                # Ensure mv_height has the expected length to avoid IndexError
                if len(mv_height) > idx:
                    obj_points[:, 2] -= mv_height[idx]
                else:
                    # Handle the case where mv_height does not have an entry for the current index (optional)
                    print(f"Warning: mv_height does not have a value for index {idx}. Skipping height adjustment for this object.")
            else:
                # Log a message or take appropriate action if mv_height is None
                print("Warning: mv_height is None. Skipping height adjustment.")
        ...
        if self.sampler_cfg.get('FILTER_OBJ_POINTS_BY_TIMESTAMP', False) or obj_points.shape[-1] != points.shape[-1]:
            if self.sampler_cfg.get('FILTER_OBJ_POINTS_BY_TIMESTAMP', False):
                ...
                if obj_points.shape[-1] != points.shape[-1] + 1:
                    raise AssertionError
    ...
    def __call__(self, data_dict):
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                if self.sampler_cfg.get('DATABASE_WITH_FAKELIDAR',
                                                False):
                    raise AssertionError('Please use latest codes to generate GT_DATABASE')
