...
def process_and_save_frame(frame_path, frame_anno, ts2idx, segname, output_dir, save_bin):
    frame_info = {'uuid': segname + '/' + frame_path.split('/')[-1].split('.')[0], 'sample_idx': ts2idx[frame_info['uuid']], 'image': {}, 'point_cloud': dict(
        num_features=4,
        velodyne_path=None,
    ), 'calib': {}, 'pose': {}, 'annos': dict(
        name=None,
        truncated=None,
        occluded=None,
        alpha=None,
...
class Argo2Dataset(DatasetTemplate):
    ...
    def evaluation(self,
        ...
        categories = {x.value for x in CompetitionCategories}
