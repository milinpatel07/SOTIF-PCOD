...
def generate_labels(frame, pose):
    ...
    for i, item in enumerate(laser_labels):
        box = item.box
        class_ind = item.type
        loc = [box.center_x, box.center_y, box.center_z]
        heading_angles.append(box.heading)
        obj_name.append(WAYMO_CLASSES[class_ind])
        difficulty.append(item.detection_difficulty_level)
        tracking_difficulty.append(item.tracking_difficulty_level)
        dimensions.append([box.length, box.width, box.height])  # lwh in unified coordinate of OpenPCDet
        locations.append(loc)
        obj_ids.append(item.id)
        num_points_in_gt.append(item.num_lidar_points_in_box)
        speeds.append([item.metadata.speed_x, item.metadata.speed_y])
        accelerations.append([item.metadata.accel_x, item.metadata.accel_y])
...
def process_single_sequence(sequence_file, save_path, sampled_interval, has_label=True, use_two_returns=True, update_info_only=False):
    ...
    for cnt, data in enumerate(dataset):
        ...
        if update_info_only and sequence_infos_old is not None:
            if info['frame_id'] != sequence_infos_old[cnt]['frame_id']:
