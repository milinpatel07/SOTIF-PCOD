...
class Box3D:
    ...
    def __init__(self, **kwargs):
        ...
        if not np.all([x > 0 for x in size]):
    ...
    def calculate_ground_bbox_coords(self):
        ...
        if not self.check_orthogonal(point_0, point_1, point_3):
            raise AssertionError
        if not self.check_orthogonal(point_1, point_0, point_2):
            raise AssertionError
        if not self.check_orthogonal(point_2, point_1, point_3):
            raise AssertionError
        if not self.check_orthogonal(point_3, point_0, point_2):
            raise AssertionError
    ...
    def get_area_intersection(self, other) -> float:
        ...
        if result > self.width * self.length:
            raise AssertionError
...
def recall_precision(gt, predictions, iou_threshold_list):
    ...
    if not np.all(recalls >= 0) & np.all(recalls <= 1):
        raise AssertionError

    # avoid divide by zero in case the first detection matches a difficult ground truth
    precisions = tp / np.maximum(tp + fp, np.finfo(np.float64).eps)

    if not np.all(precisions >= 0) & np.all(precisions <= 1):
        raise AssertionError
...
def get_average_precisions(gt: list, predictions: list, class_names: list, iou_thresholds: list) -> np.array:
    ...
    """
    if not all(0 <= iou_th <= 1 for iou_th in iou_thresholds):
        raise AssertionError
