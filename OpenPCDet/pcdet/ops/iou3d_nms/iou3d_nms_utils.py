...
def boxes_bev_iou_cpu(boxes_a, boxes_b):
    ...
    if (boxes_a.is_cuda or boxes_b.is_cuda):
        raise AssertionError('Only support CPU tensors')
    if not (boxes_a.shape[1] == 7 and boxes_b.shape[1] == 7):
...
def boxes_iou_bev(boxes_a, boxes_b):
    ...
    Returns:
        ans_iou: (N, M)
    """
    if not boxes_a.shape[1] == boxes_b.shape[1] == 7:
        raise AssertionError
