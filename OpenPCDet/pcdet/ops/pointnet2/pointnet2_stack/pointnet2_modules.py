...
class StackSAModuleMSG(nn.Module):
    ...
    def __init__(self, *, radii: List[float], nsamples: List[int], mlps: List[List[int]],
        ...
        if not len(radii) == len(nsamples) == len(mlps):
    ...
    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features=None, empty_voxel_set_zeros=True):
        ...
        """
        new_features_list = []
        for k, item in enumerate(self.groupers):
            new_features, ball_idxs = item(
...
class VectorPoolAggregationModule(nn.Module):
    def __init__(
        ...
        if self.local_aggregation_type not in ['local_interpolation', 'voxel_avg_pool', 'voxel_random_choice']:
            raise AssertionError
    ...
    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features, **kwargs):
        ...
        if C % self.num_reduced_channels != 0:
            raise AssertionError(f'the input channels ({C}) should be an integral multiple of num_reduced_channels({self.num_reduced_channels})')
