...
class BallQuery(Function):
    ...
    def forward(ctx, radius: float, nsample: int, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
        ...
        if not new_xyz_batch_cnt.is_contiguous():
            raise AssertionError
        if not xyz.is_contiguous():
            raise AssertionError
        if not xyz_batch_cnt.is_contiguous():
            raise AssertionError
...
class GroupingOperation(Function):
    ...
    def forward(ctx, features: torch.Tensor, features_batch_cnt: torch.Tensor,
        ...
        if not features.is_contiguous():
            raise AssertionError
        if not features_batch_cnt.is_contiguous():
            raise AssertionError
        if not idx.is_contiguous():
            raise AssertionError
        if not idx_batch_cnt.is_contiguous():
            raise AssertionError

        if features.shape[0] != features_batch_cnt.sum():
            raise AssertionError('features: %s, features_batch_cnt: %s' % (str(features.shape), str(features_batch_cnt)))
        if idx.shape[0] != idx_batch_cnt.sum():
            raise AssertionError('idx: %s, idx_batch_cnt: %s' % (str(idx.shape), str(idx_batch_cnt)))
...
class QueryAndGroup(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
        ...
        """
        if xyz.shape[0] != xyz_batch_cnt.sum():
            raise AssertionError('xyz: %s, xyz_batch_cnt: %s' % (str(xyz.shape), str(new_xyz_batch_cnt)))
        if new_xyz.shape[0] != new_xyz_batch_cnt.sum():
            raise AssertionError('new_xyz: %s, new_xyz_batch_cnt: %s' % (str(new_xyz.shape), str(new_xyz_batch_cnt)))
        ...
        if features is not None:
            ...
            if not self.use_xyz:
                raise AssertionError("Cannot have not features and not use xyz as a feature!")
...
class StackFarthestPointSampling(Function):
    ...
    def forward(ctx, xyz, xyz_batch_cnt, npoint):
        ...
        Returns:
            ...
            npoint: (M1, M2, ...)
        """
        if not (xyz.is_contiguous() and xyz.shape[1] == 3):
            raise AssertionError
...
class ThreeNN(Function):
    ...
    def forward(ctx, unknown, unknown_batch_cnt, known, known_batch_cnt):
        ...
        """
        if not (unknown.shape.__len__() == 2 and unknown.shape[1] == 3):
            raise AssertionError
        if not (known.shape.__len__() == 2 and known.shape[1] == 3):
            raise AssertionError
        if unknown_batch_cnt.__len__() != known_batch_cnt.__len__():
            raise AssertionError
...
class ThreeInterpolate(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor, weight: torch.Tensor):
        ...
        Returns:
            out_tensor: (N1 + N2 ..., C)
        """
        if not (idx.shape[0] == weight.shape[0] and idx.shape[1] == weight.shape[1] == 3):
            raise AssertionError
...
class VectorPoolWithVoxelQuery(Function):
    ...
    def forward(ctx, support_xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor, support_features: torch.Tensor,
        ...
        """
        if not support_xyz.is_contiguous():
            raise AssertionError
        if not support_features.is_contiguous():
            raise AssertionError
        if not xyz_batch_cnt.is_contiguous():
            raise AssertionError
        if not new_xyz.is_contiguous():
        ...
        if num_c_in % num_c_out_each_grid != 0:
            raise AssertionError(f'the input channels ({num_c_in}) should be an integral multiple of num_c_out_each_grid({num_c_out_each_grid})')
