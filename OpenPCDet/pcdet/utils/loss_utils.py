...
class SigmoidFocalClassificationLoss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, weights: torch.Tensor):
        ...
        if weights.shape.__len__() != loss.shape.__len__():
...
class SoftmaxFocalLossV2(nn.Module):
    ...
    @staticmethod
    def align_tensors(input, target, weights):
        # Assuming the mismatch is in the second dimension (number of anchors)
        _, num_preds, _ = input.shape
        _, num_targets, _ = target.shape

        if num_preds != num_targets:
            # Simple subsample or interpolation logic
            # Here we just select indices linearly spaced from the input to match the target's size
            indices = torch.linspace(0, num_preds - 1, steps=num_targets).long()
            input_aligned = input[:, indices, :]
            weights_aligned = weights[:, indices] if weights is not None else None
        else:
            input_aligned = input
            weights_aligned = weights

        return input_aligned, target, weights_aligned

    def forward(self, input: torch.Tensor, target: torch.Tensor, weights: torch.Tensor):
        # Align input and target tensors
        input_aligned, target_aligned, weights_aligned = self.align_tensors(input, target, weights)

        pred_softmax = torch.softmax(input_aligned, dim=2)
        ...
        ce_loss = -target_aligned * torch.log(torch.clamp(pred_softmax, min=1e-10))

        loss = focal_weight * ce_loss

        if weights_aligned is not None:
            if weights_aligned.shape.__len__() == 2 or \
                    (weights_aligned.shape.__len__() == 1 and target_aligned.shape.__len__() == 2):
                weights_aligned = weights_aligned.unsqueeze(-1)
            if weights_aligned.shape.__len__() != loss.shape.__len__():
                raise AssertionError

            loss = loss * weights_aligned

        return loss.mean()  # or any other reduction that fits your needs
...
class WeightedSmoothL1Loss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, weights: torch.Tensor = None):
        ...
        if weights is not None:
            if not (weights.shape[0] == loss.shape[0] and weights.shape[1] == loss.shape[1]):
                raise AssertionError
...
class WeightedCrossEntropyLoss(nn.Module):
    ...
    @staticmethod
    def forward(input: torch.Tensor, target: torch.Tensor, weights: torch.Tensor):
        """
        Args:
            input: (B, #anchors, #classes) float tensor.
...
def get_corner_loss_lidar(pred_bbox3d: torch.Tensor, gt_bbox3d: torch.Tensor):
    ...
    """
    if pred_bbox3d.shape[0] != gt_bbox3d.shape[0]:
        raise AssertionError
...
class VarRegLoss(nn.Module):
    ...
    def add_sin_difference(boxes1, boxes2, dim=6):
        print(f"Before add_sin_difference - boxes1 shape: {boxes1.shape}, boxes2 shape: {boxes2.shape}")
        if dim == -1:
            raise AssertionError
    ...
    def forward(self, reg_preds: torch.Tensor, var_preds: torch.Tensor, gt_targets: torch.Tensor, \
        ...
        print(f"VarRegLoss - reg_preds shape: {reg_preds.shape}, gt_targets shape: {gt_targets.shape}")
        """
        Args:
...
class RegLossCenterNet(nn.Module):
    ...
    @staticmethod
    def forward(output, mask, ind=None, target=None):
        """
        Args:
...
class RegLossSparse(nn.Module):
    ...
    @staticmethod
    def forward(output, mask, ind=None, target=None, batch_index=None):
        """
        Args:
            output: (N x dim)
            mask: (batch x max_objects)
...
class IouLossSparse(nn.Module):
    ...
    @staticmethod
    def forward(iou_pred, mask, ind, box_pred, box_gt, batch_index):
        if mask.sum() == 0:
