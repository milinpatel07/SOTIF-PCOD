...
class PointHeadTemplate(nn.Module):
    ...
    def assign_stack_targets(self, points, gt_boxes, extend_gt_boxes=None,
        ...
        """
        if not (len(points.shape) == 2 and points.shape[1] == 4):
            raise AssertionError('points.shape=%s' % str(points.shape))
        if not (len(gt_boxes.shape) == 3 and gt_boxes.shape[2] == 8):
            raise AssertionError('gt_boxes.shape=%s' % str(gt_boxes.shape))
        if not (extend_gt_boxes is None or len(extend_gt_boxes.shape) == 3 and extend_gt_boxes.shape[2] == 8):
            raise AssertionError('extend_gt_boxes.shape=%s' % str(extend_gt_boxes.shape))
        if set_ignore_flag == use_ball_constraint:
            raise AssertionError('Choose one only!')
