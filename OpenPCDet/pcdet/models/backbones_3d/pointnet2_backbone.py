...
class PointNet2MSG(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        if xyz_batch_cnt.min() != xyz_batch_cnt.max():
            raise AssertionError
        xyz = xyz.view(batch_size, -1, 3)
        features = features.view(batch_size, -1, features.shape[-1]).permute(0, 2, 1).contiguous() if features is not None else None

        l_xyz, l_features = [xyz], [features]
        for i, item in enumerate(self.SA_modules):
            li_xyz, li_features = item(l_xyz[i], l_features[i])
            l_xyz.append(li_xyz)
...
class PointNet2Backbone(nn.Module):
    ...
    def __init__(self, model_cfg, input_channels, **kwargs):
        if not False:
            raise AssertionError('DO NOT USE THIS CURRENTLY SINCE IT MAY HAVE POTENTIAL BUGS, 20200723')
        super().__init__()
        self.model_cfg = model_cfg
