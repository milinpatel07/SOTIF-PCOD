...
def post_processing_var(self, batch_dict):
    ...
    for index in range(batch_size):
        if batch_dict.get('batch_index', None) is not None:
            if batch_dict['batch_box_preds'].shape.__len__() != 2:
                raise AssertionError
            batch_mask = (batch_dict['batch_index'] == index)
        else:
            if batch_dict['batch_box_preds'].shape.__len__() != 3:
                raise AssertionError
            batch_mask = index
        ...
        if not isinstance(batch_dict['batch_cls_preds'], list):
            ...
            elif clf_loss_name in ('SoftmaxFocalLossV1', 'SoftmaxFocalLossV2'):
                num_classes = self.num_class 
            print("cls_preds shape:", cls_preds.shape)
            print("Adjusted num_classes:", num_classes)
            print("Expected num_classes:", num_classes)

            if cls_preds.shape[1] not in [1, num_classes]:
                raise AssertionError
...
def post_processing_mimo(self, batch_dict):
    ...
    for batch_index in range(mimo_batch_size):
        ...
        for cluster_num in range(total_num_clusters):
            ...
            if torch.max(hist) != num_preds_in_cluster:
                ...
                if num_preds_in_cluster in (2, 3):
                    continue
    ...
    for i, item in enumerate(pred_dicts_list):
        pred_dicts[0]['pred_dicts_list'].append(item)
