...
class MPPNetHeadE2E(RoIHeadTemplate):
    ...
    @staticmethod
    def crop_current_frame_points(src, batch_size,trajectory_rois,num_rois,num_sample, batch_dict):
    ...
    @staticmethod
    def transform_prebox_to_current_vel(pred_boxes3d,pose_pre,pose_cur):
    ...
    @staticmethod
    def generate_trajectory(cur_batch_boxes,proposals_list,batch_dict):
    ...
    @staticmethod
    def reorder_memory(memory,num_max_rois):

        ordered_memory = memory[0].new_zeros([len(memory),num_max_rois,memory[0].shape[1],memory[0].shape[2]])
        for bs_idx, item in enumerate(memory):
            ordered_memory[bs_idx,:len(item)] = item
