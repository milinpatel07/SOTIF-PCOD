...
def ece(gt, pred, iou_thres, num_bins=10, labels=['Car', 'Truck', 'Pedestrian']):
    ...
    for frame_idx, item in enumerate(pred):
        gt_label = gt[frame_idx]['label']
        gt_box = gt[frame_idx]['box']
        pred_label = item['name']
        pred_box = item['boxes_lidar']
        pred_score = item['score']
        ...
        for label in labels:
            ...
            for i, item in enumerate(iou):
                ...
                if item >= iou_thres and pred_label_masked[i] == gt_label_masked[gt_index[i]]:
