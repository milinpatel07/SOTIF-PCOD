...
class SigmoidFocalClassificationLoss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, weights: torch.Tensor):
        ...
        # Interpolate to match the spatial dimensions of target
        pred_sigmoid = F.interpolate(pred_sigmoid, size=(target.shape[1], target.shape[2]), mode='nearest')
       # pred_sigmoid = F.interpolate(pred_sigmoid.unsqueeze(2), size=(target.shape[1], 1, target.shape[2]), mode='nearest').squeeze(2)
        
        alpha_weight = target * self.alpha + (1 - target) * (1 - self.alpha)
        print("Shape of target tensor:", target.shape)
        print("Shape of pred_sigmoid tensor:", pred_sigmoid.shape)
...
class SoftmaxFocalLossV1(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, weights: torch.Tensor):
        ...
        # Adjust pred_softmax shape to match the shape of target
        batch_size = pred_softmax.shape[0]
        num_anchors = pred_softmax.shape[1]
        num_classes = target.shape[2]
        pred_softmax = pred_softmax.view(batch_size, -1, num_classes)  # Reshape to match target
        ...
        if weights.ndim == 2 or \
                (weights.ndim == 1 and target.ndim == 2):
            weights = weights.unsqueeze(-1)

        if weights.shape != loss.shape:
            raise AssertionError("Weights shape does not match loss shape")
...
class SoftmaxFocalLossV2(nn.Module):
    ...
    def align_tensors(input, target, weights):
        ...
        if num_preds != num_targets:
            # Resample input and weights to match target size
            indices = torch.linspace(0, num_preds - 1, steps=num_targets).long()
            input = input[:, indices, :]
            if weights is not None:
                weights = weights[:, indices]
        
        return input, target, weights

    def forward(self, input: torch.Tensor, target: torch.Tensor, weights: torch.Tensor):
        # Align input and target tensors
        input, target, weights = self.align_tensors(input, target, weights)

        # Compute softmax and focal weight
        ...
        if weights is not None:
            # Ensure weights have the same shape as loss
            if weights.ndim == 2:
                weights = weights.unsqueeze(-1)
            if weights.shape != loss.shape:
                raise AssertionError("Weights shape does not match loss shape")
        ...
        return loss.mean()  # Reduce to obtain scalar loss
